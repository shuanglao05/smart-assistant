...
### 21.1 分片算法重写：从「定长窗口」到「语义单元合并」

**问题（实测定位）**
旧 `split_text()` 是**定长滑窗**：从起点硬推 500 字符 → 在窗内 `rfind` 找
`\n\n` / `\n` / `。` / `. ` 中最靠后的断点 → 断点若未过半（`cut <= size*0.5`）
就放弃、原样在第 500 字符处**硬切** → 下一段起点 = 本段末尾 **直接回退 80 字符**。

关键缺陷在最后一步：回退位置落在上一段的句子内部，**等于每次重叠都把一句话劈成两半**。
用库内 5 份真实文档（3 部小说 txt + 2 份校规 docx）量化：

| 指标 | 旧算法 | 新算法 |
|---|---|---|
| 片段以标点开头（上一句被腰斩） | **10.39%** | **0.36%** |
| 片段尾部未完 | 1.05% | 0.81% |
| 片段总数 | 1242 | 1107 |

（统计口径：片段首字符不是字母/数字/引号/编号即为「腰斩」；
不含 `…` `——` 这类句中停顿符号。）

**新实现（`rag.py`）**
1. `_split_units()` 先把正文拆成**最小语义单元**：空行分段 → 换行分行 →
   小标题（`第X章`/`一、`/`（一）`/`1.1`/`附录A`/`#`）单独成单元 → 其余行按句切分。
   单元内部**保证是完整句子**。
2. `split_text()` 贪心合并单元：加下一个不超 `size` 就继续加，攒满一片。
3. **重叠改为「回退若干完整单元」**而非固定字符数——从下一片起点往前捡回单元
   直到累计接近 `overlap`，因此重叠区域同样是完整句子，不会再制造半句话。
4. 单个语义单元自身超长（整段无句号的说明文字）时，用 `_split_long_unit()`
   按逗号/顿号二次切分，避免产生巨型片段；切下的余量替换回单元列表继续参与合并。
5. `size` / `overlap` 参数语义与 `.env` 配置项**完全兼容**，无需改配置。

**踩坑记录**
- 中文的 `…` 和 `——` **不能**当句末符。小说语料里 `"怎么好像…地狱列车一样？"`
  这类句中停顿极常见，误判为句末会产生以 `…` 开头的碎片（实测 14/350 片）。
  正确做法：句子终止符只取 `。！？!?；;`，并用 `_DANGLING` 单独处理「整行只有停顿符号」
  的情况（贴回上一句，不独立成单元）。
- 小标题判定**必须限长**（`len <= 40`）。否则 `第一条 为了维护……`
  这类条文正文也会命中 `第一条` 特征被误判为标题，反而把句子截断。

### 21.2 PDF 抽取后的换行归一化 `_clean_pdf_text()`

**为什么需要**：pypdf 按页面**视觉排版**抽文字，一行到纸张右边缘就换行，
一句话常被拆成两三行。这会让分片器优先命中这些「假换行」而把句子切碎，
同时也丢失段落结构。

**处理规则**（中文与英文的换行特征相反，必须分开判断）：
- 剔除页眉页脚：`第 N 页` / `- N -` / `Page N of M`；
- 英文断词合并：行尾 `-` 且下一行以小写字母开头 → `postgrad-\nuate` → `postgraduate`；
- 上一行以「明显未结束」字符收尾（`，、：；（《"—-` 等）→ 接回同一段；
- 上一行**是短标题**（`_looks_like_heading`，限长 30）或本行是短标题 → 断段；
- 上一行已完整收句（`。！？.` 等）→ 断段；
- 其余（典型：英文行末无标点的 word wrap、中文行末无标点的版式换行）→ 接回同一段，
  中英之间补空格、中文之间直连（`_join_lines()`）。

**验证**：手写多行排版 PDF 走 `_extract_text()` 全链路，确认
「依法治校」「研究生。」等被版式换行拆断的句子全部接回、
`international student` 断词修复、页码剔除、章节标题保留。

**注意**：`.txt` / `.docx` / `.md` **不做**此清洗——它们的换行本来就是语义换行，
清洗反而会破坏结构。仅 PDF 走这条路径。

### 21.3 存量索引重建

改算法后旧片段不会自动更新，需重建。两种方式：
- 单文件：`POST /api/files/{file_id}/reindex`；
- 批量：调用 `rag.index_file(user_id, file_id, content, collection_id)` 遍历 `files` 表
  （`index_file` 内部会先删除该文档的旧片段，可重复执行）。

### 21.4 SQLite 开启 WAL（并发读写）

**问题**：批量重建索引（长时间连续写入 3000+ 条 embedding）时，其他请求会直接报
`sqlite3.OperationalError: database is locked` —— 连 `init_db()` 的启动语句都跑不过。
原因是 SQLite 默认 `journal_mode=DELETE`，**写事务会阻塞读**。

**改法**（`database.py`，用 SQLAlchemy 事件在每次连接建立时挂 PRAGMA）：
```python
@event.listens_for(engine, "connect")
def _sqlite_pragmas(dbapi_conn, _record):
    cur = dbapi_conn.cursor()
    cur.execute("PRAGMA journal_mode=WAL")     # 读写并发，写不再阻塞读
    cur.execute("PRAGMA synchronous=NORMAL")   # WAL 推荐搭配，兼顾安全与速度
    cur.execute("PRAGMA busy_timeout=30000")   # 万一撞锁等 30s 而非立刻报错
```
同时把 `connect_args` 的 `timeout` 设为 30s。仅在 `DATABASE_URL` 以 `sqlite` 开头时挂载，
不影响将来切到 PostgreSQL 的场景。

**⚠️ WAL 的副作用：外部工具可能读到"旧数据"**
开启 WAL 后，写入会先进 `app.db-wal`，之后再合并（checkpoint）回主 `app.db`。
如果第三方数据库工具**不认识 WAL**（只读主文件、不读 `-wal`），就会看到删除/修改**之前**的旧快照。
SQLite 官方的 DB Browser、DBeaver 等都能正确处理 WAL，但某些简易工具或旧的查看器会踩这个坑。

**排查方法**（对比两种读法即可确认）：
```python
# 忽略 WAL（模拟认知不全的工具）
sqlite3.connect("file:app.db?mode=ro&immutable=1", uri=True)
# 正常读取（会合并 WAL）
sqlite3.connect("app.db")
```
两者结果不一致 → 就是 WAL 未合并导致的视角差异，**数据本身没问题**。

**手动合并**（让任何工具都能读到最新数据）：
```python
con = sqlite3.connect("app.db")
con.execute("PRAGMA wal_checkpoint(TRUNCATE)")   # 全部合并回主库并清空 WAL
```
